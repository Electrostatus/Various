...
use_pyqt = True  # False to use PySide, True to use PyQt4
if use_pyqt:
...
class colorPicker(QtGui.QDialog):
    ...
    def setColor(self, qcolor=None):
        if qcolor is None: self._color = QtGui.QColor('#ffffff')
        else: self._color = QtGui.QColor(qcolor)
        self._colorEdited()
    ...
    def setOption(self, option, Bool=True):
        if option == QtGui.QColorDialog.NoButtons:
            if not Bool:
    ...
    def _colorEdited(self):
        ...
        if sender in self.rgbInputs:       # RGB
            ...
            try: color = self._color = QtGui.QColor(dat.toString())  # PyQt
            except: color = self._color = QtGui.QColor(str(dat))     # PySide
    ...
    def pickColor(self):
        ...
        desktop = QtGui.QApplication.desktop().winId() # screenshot the desktop
        self._img = QtGui.QPixmap.grabWindow(desktop, 0, 0, -1, -1)

        self._view = QtGui.QGraphicsView(self)
        scene = QtGui.QGraphicsScene(self)  # display screenshot at full size
        ...
        self._appview = QtGui.QApplication
    ...
    def randomColor(self):
        ...
        rand = random.randint; col = QtGui.QColor()
    ...
    def addNamedColors(self, lst):
        ...
        for i, j in sorted(uni, key=lambda q: q[1].getHsv()):
            icon = QtGui.QPixmap(16, 16); icon.fill(j)
    ...
    def setup(self):
        ...
        # all the input boxes
        self.inputGrid = QtGui.QGridLayout()
        self.inputGrid.setSpacing(5)
        h_spc = self.inputGrid.horizontalSpacing()
        v_spc = self.inputGrid.verticalSpacing()

        # RGB
        self.redInput = QtGui.QSpinBox()
        self.grnInput = QtGui.QSpinBox()
        self.bluInput = QtGui.QSpinBox()
        self.rgbInputs = [self.redInput, self.grnInput, self.bluInput]

        self.redLabel = QtGui.QLabel('&R:'); self.redLabel.setToolTip('Red')
        self.grnLabel = QtGui.QLabel('&G:'); self.grnLabel.setToolTip('Green')
        self.bluLabel = QtGui.QLabel('&B:'); self.bluLabel.setToolTip('Blue')
        ...
        # HSV
        self.hueInput = QtGui.QSpinBox()
        self.satInput = QtGui.QSpinBox()
        self.valInput = QtGui.QSpinBox()
        self.hsvInputs = [self.hueInput, self.satInput, self.valInput]

        self.hueLabel = QtGui.QLabel('&H:'); self.hueLabel.setToolTip('Hue')
        self.satLabel = QtGui.QLabel('&S:'); self.satLabel.setToolTip('Saturation')
        self.valLabel = QtGui.QLabel('&V:'); self.valLabel.setToolTip('Value')
        ...
        # CMYK
        self.cInput = QtGui.QSpinBox()
        self.mInput = QtGui.QSpinBox()
        self.yInput = QtGui.QSpinBox()
        self.kInput = QtGui.QSpinBox()
        self.cmykInputs = [self.cInput, self.mInput, self.yInput, self.kInput]

        self.cLabel = QtGui.QLabel('&C:'); self.cLabel.setToolTip('Cyan')
        self.mLabel = QtGui.QLabel('&M:'); self.mLabel.setToolTip('Magenta')
        self.yLabel = QtGui.QLabel('&Y:'); self.yLabel.setToolTip('Yellow')
        self.kLabel = QtGui.QLabel('&K:'); self.kLabel.setToolTip('Black')
        ...
        # HTML
        self.htmlInput = QtGui.QLineEdit()
        ...
        for i in self.inputLabels:
            ...
            #i.setFrameShape(QtGui.QFrame.Box)
        ...
        # color display
        self.colorDisplay = QtGui.QFrame()
        self.colorDisplay.setFixedSize(55, 4 * 22 + 3 * v_spc)  # spans 4 rows
        self.colorDisplay.setFrameShape(QtGui.QFrame.Panel)
        self.colorDisplay.setFrameShadow(QtGui.QFrame.Sunken)
        self.colorDisplay.setAutoFillBackground(True)

        # show button / random button
        self.showButton = QtGui.QPushButton('Sho&w')
        ...
        self.rndButton = QtGui.QPushButton('R&and')
        ...
        # ok/cancel buttons
        self.dialogButtons = QtGui.QDialogButtonBox(QtGui.QDialogButtonBox.Ok
        ...
        ico = self.style().standardIcon(QtGui.QStyle.SP_DialogResetButton)
...
class wheel(QtGui.QWidget):
    ...
    def getColor(self):
        col = QtGui.QColor(); col.setHsv(self.hue, self.sat, self.value)
        return col
    ...
    def eventFilter(self, source, event):
        if (event.type() == QtCore.QEvent.MouseButtonPress or
            ...
            if self.vArcPath.contains(pos):  # in the value selection arc
                ...
                self.vIdCen = QtCore.QPointF(x, y); self.vIdBox.moveCenter(self.vIdCen)
    ...
    def paintEvent(self, event):
        ...
        color = QtGui.QColor(); color.setHsv(self.hue, self.sat, 255)
